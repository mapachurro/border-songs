  ...
  for (const folder of folders) {
    ...
    const files = (await fs.readdir(sectionPath))
      .filter(
        (f) =>
          f.endsWith(".md") &&
          !f.startsWith("track-list") &&
          !f.startsWith("introduction") &&
          /^\d+-/.test(f),
      )
      .sort();
    ...
    try {
      ...
      if (sections.videoSource) {
        ...
        if (videoId) {
          ...
          console.warn(
            `Could not extract YouTube video ID from: ${sections.videoSource}`,
          );
        }
      }
      ...
      const outputHtml = await renderTemplate(contentTemplate, {
        ...
        TRANSLATION_HTML: sections.target
          ? marked.parse(sections.target)
          : "<p>Translation content missing</p>",
        SOURCE_HTML: sections.source
          ? marked.parse(sections.source)
          : "<p>Source content missing</p>",
        COMMENTARY_HTML: marked.parse(
          (sections.commentary || "") + "\n\n" + (sections.notes || ""),
        ),
        PREV_BUTTON: "",
        NEXT_BUTTON: "",
      ...
      console.error(
        `Error building content page ${songPages[i]?.outputPath || "unknown"}:`,
        error,
      );
    }
  }
...
async function buildTitlePage() {
  const titleMd = await fs.readFile(
    path.join(binderDir, "title-page.md"),
    "utf-8",
  );
...
async function buildTrackListPages() {
  ...
  for (const folder of folders) {
    ...
    const files = (await fs.readdir(sectionPath)).filter(
      (f) => f.startsWith("track-list") && f.endsWith(".md"),
    );
...
async function buildIntroductionPage() {
  try {
    ...
    const introMd = await fs.readFile(
      path.join(binderDir, "introduction.md"),
      "utf-8",
    );

    // Don't use splitMarkdownSections here since the introduction format is different
    const introHtml = await renderTemplate(
      path.resolve(__dirname, "intro-template.html"),
      {
        ASSET_PATH: "",
        TITLE: "Introduction",
  ...
  try {
    ...
    for (const folder of folders) {
      ...
      try {
        ...
        await fs.mkdir(outputDir, { recursive: true });
        await fs.writeFile(
          path.join(outputDir, "introduction.html"),
          introHtml,
        );
        ...
        console.error(
          `Error building introduction for section ${folder}:`,
          error,
        );
      }
    }
...
async function buildAll() {
  ...
  navIndex.sort((a, b) => {
    ...
    if (a === "introduction.html")
      return b === "index.html" || b === "toc.html" ? 1 : -1;
    if (b === "introduction.html")
      return a === "index.html" || a === "toc.html" ? -1 : 1;
    ...
    const aNum = parseInt(
      aFile.match(/^(\d+)-/) ? aFile.match(/^(\d+)-/)[1] : "999",
      10,
    );
    const bNum = parseInt(
      bFile.match(/^(\d+)-/) ? bFile.match(/^(\d+)-/)[1] : "999",
      10,
    );
