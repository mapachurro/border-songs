...
export async function getTrackLists(markdownLinkStyle = false) {
  const files = (await glob("./binder/**/track-list-*.md")).sort((a, b) => {
    const aNum = parseInt(
      path.basename(a).match(/track-list-(\d+)/)?.[1] || 0,
      10,
    );
    const bNum = parseInt(
      path.basename(b).match(/track-list-(\d+)/)?.[1] || 0,
      10,
    );
    return aNum - bNum;
  });
...
export async function getOrderedSongPages() {
  ...
  for (const filePath of files) {
    ...
    const mdFiles = (await fs.readdir(path.join("binder", folder))).filter(
      (f) => f.endsWith(".md") && !f.startsWith("track-list"),
    );
